# MacSlowCooker Implementation Plan
...
│   ├── MacSlowCooker.entitlements
│   ├── Info.plist                       # SMPrivilegedExecutables キー含む
│   └── Library/
│       └── LaunchDaemons/
│           └── com.macslowcooker.helper.plist  # launchd daemon plist (bundle同梱)
...
**Files:**
- Create: `MacSlowCooker.xcodeproj` (Xcode GUIで作成)
- Create: `MacSlowCooker/MacSlowCooker.entitlements`
...
2. Product Name: `HelperTool`, Bundle Identifier: `com.macslowcooker.helper`
3. Language: Swift

- [ ] **Step 3: MacSlowCookerTestsターゲットを確認**

プロジェクト作成時に自動生成された `MacSlowCookerTests` ターゲットが存在することを確認。なければ File → New → Target → macOS → Unit Testing Bundle で追加。

- [ ] **Step 4: 両ターゲットのDeployment Targetを設定**

Project Settings → MacSlowCooker target → Deployment Info → macOS `14.0`
Project Settings → HelperTool target → macOS `14.0`
Project Settings → MacSlowCookerTests target → macOS `14.0`

- [ ] **Step 5: Sharedグループを作成しターゲットメンバーシップを設定**

Xcode Navigator でグループ `Shared` を作成（後のタスクでファイルを追加する際、`MacSlowCooker` と `HelperTool` 両ターゲットにチェックを入れる）

- [ ] **Step 6: entitlementsファイルを作成**

`MacSlowCooker/MacSlowCooker.entitlements`:
```xml
...
<dict>
    <key>Label</key>
    <string>com.macslowcooker.helper</string>
    <key>MachServices</key>
    <dict>
        <key>com.macslowcooker.helper</key>
        <true/>
    </dict>
...
Xcodeで `com.macslowcooker.helper.plist` を `MacSlowCooker` ターゲットに追加し、Build Phases → Copy Bundle Resources に含まれていることを確認。

- [ ] **Step 8: HelperToolバイナリをapp bundleにコピーするBuild Phaseを追加**

MacSlowCooker target → Build Phases → `+` → New Copy Files Phase:
- Destination: `Wrapper`
...
- Create: `MacSlowCookerTests/GPUSampleTests.swift`

- [ ] **Step 1: テストを先に書く**

`MacSlowCookerTests/GPUSampleTests.swift`:
```swift
import XCTest
...
`Shared/GPUSample.swift` (ターゲット: MacSlowCooker + HelperTool):
```swift
import Foundation
...
`Shared/XPCProtocol.swift` (ターゲット: MacSlowCooker + HelperTool):
```swift
import Foundation
...
- Create: `MacSlowCookerTests/PowerMetricsParserTests.swift`

- [ ] **Step 1: パーステストを先に書く**

`MacSlowCookerTests/PowerMetricsParserTests.swift`:
```swift
import XCTest
@testable import MacSlowCooker
...
```bash
git add HelperTool/PowerMetricsRunner.swift MacSlowCookerTests/PowerMetricsParserTests.swift
...
<dict>
    ...
    <array>
        ...
        <string>identifier "com.macslowcooker.app" and anchor apple generic and certificate 1[field.1.2.840.113635.100.6.2.6] exists and certificate leaf[field.1.2.840.113635.100.6.1.13] exists and certificate leaf[subject.OU] = "XXXXXXXXXX"</string>
    </array>
</dict>
</plist>
```
...
**Files:**
- Create: `MacSlowCooker/GPUDataStore.swift`
- Create: `MacSlowCookerTests/GPUDataStoreTests.swift`

- [ ] **Step 1: テストを先に書く**

`MacSlowCookerTests/GPUDataStoreTests.swift`:
```swift
import XCTest
@testable import MacSlowCooker
...
`MacSlowCooker/GPUDataStore.swift`:
```swift
import Foundation
import Observation
...
```bash
git add MacSlowCooker/GPUDataStore.swift MacSlowCookerTests/GPUDataStoreTests.swift
...
**Files:**
- Create: `MacSlowCooker/XPCClient.swift`

- [ ] **Step 1: XPCClient.swiftを実装する**

`MacSlowCooker/XPCClient.swift`:
```swift
import Foundation
import os.log
...
final class XPCClient {
    ...
    private func makeConnection() {
        let conn = NSXPCConnection(machServiceName: "com.macslowcooker.helper", options: [])
...
```bash
git add MacSlowCooker/XPCClient.swift
...
**Files:**
- Create: `MacSlowCooker/HelperInstaller.swift`
- Modify: `MacSlowCooker/Info.plist`

- [ ] **Step 1: MacSlowCooker/Info.plistにSMPrivilegedExecutablesキーを追加**

`MacSlowCooker/Info.plist` に以下を追加（既存のdict内）:
```xml
<key>SMPrivilegedExecutables</key>
<dict>
    <!-- XXXXXXXXXX を実際のTeam IDに置換 -->
    <key>com.macslowcooker.helper</key>
    <string>identifier "com.macslowcooker.helper" and anchor apple generic and certificate 1[field.1.2.840.113635.100.6.2.6] exists and certificate leaf[field.1.2.840.113635.100.6.1.13] exists and certificate leaf[subject.OU] = "XXXXXXXXXX"</string>
</dict>
```

- [ ] **Step 2: HelperInstaller.swiftを実装する**

`MacSlowCooker/HelperInstaller.swift`:
```swift
import Foundation
    ...
    static func installIfNeeded() async throws {
        ...
        case .notFound:
            ...
            throw HelperInstallerError.registrationFailed(
                NSError(domain: "com.macslowcooker", code: -1, userInfo: [NSLocalizedDescriptionKey: "Daemon plist not found"])
            )
...
```bash
git add MacSlowCooker/HelperInstaller.swift MacSlowCooker/Info.plist
...
**Files:**
- Create: `MacSlowCooker/DockIconRenderer.swift`
- Create: `MacSlowCookerTests/DockIconRendererTests.swift`

- [ ] **Step 1: テストを先に書く**

`MacSlowCookerTests/DockIconRendererTests.swift`:
```swift
import XCTest
@testable import MacSlowCooker
...
`MacSlowCooker/DockIconRenderer.swift`:
```swift
import AppKit
import CoreGraphics
...
```bash
git add MacSlowCooker/DockIconRenderer.swift MacSlowCookerTests/DockIconRendererTests.swift
...
**Files:**
- Create: `MacSlowCooker/PopupView.swift`

- [ ] **Step 1: PopupView.swiftを実装する**

`MacSlowCooker/PopupView.swift`:
```swift
import SwiftUI
import Charts
...
```bash
git add MacSlowCooker/PopupView.swift
...
**Files:**
- Create: `MacSlowCooker/PopupWindowController.swift`

- [ ] **Step 1: PopupWindowController.swiftを実装する**

`MacSlowCooker/PopupWindowController.swift`:
```swift
import AppKit
import SwiftUI
...
```bash
git add MacSlowCooker/PopupWindowController.swift
...
**Files:**
- Create: `MacSlowCooker/AppDelegate.swift`

- [ ] **Step 1: AppDelegate.swiftを実装する**

`MacSlowCooker/AppDelegate.swift`:
```swift
import AppKit
import os.log
...
2. macOSが「MacSlowCookerが変更を加えようとしています」ダイアログを表示
3. 管理者パスワードを入力
...
```bash
git add MacSlowCooker/AppDelegate.swift MacSlowCooker/Info.plist
git commit -m "feat: wire AppDelegate — MacSlowCooker v1.0 complete"
```
