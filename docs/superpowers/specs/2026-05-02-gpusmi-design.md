...
- Bundle ID: `com.gpusmi.helper`
- インストール先: `/Library/PrivilegedHelperTools/com.gpusmi.helper`
- 起動方法: launchd（`SMAppService.daemon` 登録後は自動起動）
- LaunchDaemon plist: `GPUSMI/Library/LaunchDaemons/com.gpusmi.helper.plist` をapp bundleに同梱
...
| ターゲット | 要件 |
|-----------|------|
| GPUSMI.app | Developer ID Application で署名 |
| HelperTool | Developer ID Application で署名（同一チームID） |
| GPUSMI.app Info.plist | `SMPrivilegedExecutables` に `com.gpusmi.helper` とコード署名要件を記載 |
...
│       └── com.gpusmi.helper.plist  — bundleに同梱するlaunchd plist
├── HelperTool/
...
`os_log` (Unified Logging System) を使用し、サブシステム `com.gpusmi` でカテゴリ分け:
