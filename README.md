...
- 蓋から立ち昇る **〰️ 波形の湯気** がファン RPM (ファン搭載機) または SoC 温度 (ファンレス機) に連動して本数・太さ・高さが変化
...
- **チャート**: GPU / Temperature / Fan / Power の 4 列 (ファンレス機種では Fan を除く 3 列)
- **メトリクスタイル**: 数値が**危険度に応じて白→黄→赤**に変化 (ファンレス機種では Fan を除く 3 種)
...
- **ファンレス機種対応** (MacBook Air M-series など): Fan チャート・Fan メトリクスタイルは自動非表示、湯気は温度ベースにフォールバック
- Intel Mac は非対応
